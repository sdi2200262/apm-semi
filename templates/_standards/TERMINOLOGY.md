...
| **Task** | Discrete work unit with objective, deliverables, validation criteria, and dependencies. Tasks contain ordered sub-units (steps) that support failure tracing but have no independent validation. Each Task carries a current owner - either an AI Worker (the default) or the User. |
...
**Collaborating agent.** The AI agent on the User's side when the User holds a Task. The Manager is the collaborating agent for Tasks the Plan assigned to the User and for Tasks the User claims during Manager coordination. The Worker assigned to a Task in progress becomes the collaborating agent when the User takes over in the Worker's chat - the Worker pauses execution and stays on standby. The collaborating agent answers questions, runs validation when the User returns, and writes the Task Log on the User's behalf. While the User holds the Task, the collaborating agent does not execute Task work autonomously.

**Sovereignty signal.** An indication during Context Gathering that the User wants direct ownership over part of the work - a domain preference, an area the User has stressed about, something the User wants full personal knowledge of, a part described with strong personal language, or an explicit statement that the User intends to do certain work themselves. APM Semi's typical pattern is the User taking on critical or high-stakes work and AI Workers handling boilerplate or peripheral Tasks - sovereignty signals shape how the Planner reads and advises on Worker-vs-User assignment. Signals surface organically through the Planner's existing question rounds and carry into Plan Analysis - signals that translate into User-owned Tasks are captured by those Tasks; signals that don't translate (borderline, partial, or contextual) are recorded in Plan notes for the Manager to read during the Implementation Phase.

**User claim.** The User taking ownership of a Task, in any of three forms: at planning time during Plan review (the Plan records the User as assignee); dynamically during the Implementation Phase in the Manager's chat for an unassigned Task; or takeover - the User stepping into a Worker's chat where a Task is currently being executed and signaling so, which causes the Worker to pause cleanly without writing a partial Task Log. The User may also unclaim a Task they hold, in which case progress so far becomes context the Manager packages into a Task Prompt for the responsible Worker (or, if no Worker was originally assigned, a best-fit Worker from the registry). All claim forms and unclaim are conversational - no command exists for any of them.
