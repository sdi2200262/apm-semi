...
**User claims during review.** When the Planner presents the Plan for review, the User may choose to claim Tasks directly. Claimed Tasks are assigned to the User in the Plan instead of an AI Worker. The Spec is not modified by claims as a side effect - the Spec captures project decisions, not execution distribution. Rationale concerning work-structure observation belongs in Plan notes; rationale concerning project-level reasoning may warrant a Spec update with the rationale recorded in Spec header notes. If the User claims no Tasks at planning, the Plan looks identical to a non-collaborative Plan and the Implementation Phase begins as a normal session - until and unless the User claims something dynamically later.
