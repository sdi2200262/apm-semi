...
**User-owned Task path** - When a User-owned Task becomes Ready, the Manager does not dispatch to a Task Bus. Instead, in the Manager's chat, it presents a Task Brief built from the same three-source synthesis as a Task Prompt (relevant Spec content, dependency context, Plan Task fields including objective, steps, guidance, output, and validation criteria), but written conversationally as a brief rather than as instructions to an executor. The Task Brief carries everything the User would need to do the work. The Manager then stays on standby for that Task while continuing to coordinate AI Workers on independent Tasks in parallel. The same Brief construction applies on the Worker side when the User takes over mid-execution per §7.2 Task Execution - the Worker presents a Brief with execution progress baked in instead of starting from scratch. See `{GUIDE_PATH:task-assignment}` for Task Brief construction.
