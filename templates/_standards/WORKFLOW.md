...
**Collaborative ownership** - The User is the project owner. Workers are the implementation team and execute Tasks dispatched by the Manager. The User may claim any Task at any point and execute it as the owner stepping in. Whichever agent currently has context for a claimed Task - the Manager for unassigned or Manager-hosted Tasks, the Worker for in-progress Tasks - hosts the User's execution as a standby collaborator: answering questions, running validation when the User returns, and writing the Task Log on the User's behalf. The User is not a Worker - no bus slot, no Worker registry entry, no Task Prompt delivered, no Rules to follow, and no Handoff procedure. When the User does not claim Tasks, full Manager-Worker dispatch operates unchanged.
...
The Manager simultaneously maintains two postures. Toward Workers, it is a dispatcher - writing Task Prompts to Task Buses, reading Task Reports, reviewing Task Logs. Toward the User, it is a direct collaborator in chat with no message bus between them - answering questions, surfacing recommendations, hosting User-claimed Tasks. The two postures run in parallel. Coordination of AI Workers continues while the Manager hosts a User-claimed Task. A Worker may also enter a collaborator posture when the User takes over an in-progress Task in the Worker's chat.
...
The Plan accepts the User as a valid Task assignee. When the User claims a Task at planning time, the Plan records the assignee as the User and the Worker that would have owned it does not gain a Worker registry entry on the User's behalf. If a Worker ends up with no Tasks because the User claimed everything in its domain, the Planner removes that Worker from the registry. The User is never added to the Worker registry. Dependency graph, Stage structure, and parallel-dispatch logic are unchanged for User-owned Tasks - they participate in dependencies and parallel-dispatch decisions exactly like Worker-owned Tasks; the Manager simply hosts them in chat instead of dispatching to a bus.
...
When the User claims a Task that has not yet been dispatched (including Tasks claimed at planning time), the Manager hosts execution in its own chat. The Manager builds a Task Brief from the same three sources used for a Task Prompt - relevant Spec content, dependency context, and Plan Task fields - but presents it conversationally as a brief rather than as instructions to an executor. The Task Brief carries everything the User needs to do the work. While the User executes, the Manager is a standby collaborator - available for questions, providing coordination-level context (cross-Worker contracts, Spec ambiguities, dependency implications, Stage timing), and continuing to coordinate AI Workers on independent Tasks in parallel. The Task is held in Active state with an ownership marker for the User; the User may pause and resume freely.

When the User takes over a Task that a Worker is currently executing, the Worker recognizes the takeover signal in its own chat and pauses cleanly. It does not write a partial Task Log and does not report the takeover to the Manager at takeover time - the Manager learns when the eventual Task Report and Task Log arrive, which include a marker noting that a takeover occurred and at what point. The Worker enters the same standby collaborator posture the Manager uses for User-owned Tasks: available for execution-level questions (what was done so far, files touched, decisions made, what was about to come next), running validation when the User returns, and writing the Task Log on the User's behalf. Manager-hosted and Worker-hosted collaborative execution are identical in shape; only the flavor of context the host can offer differs.
...
**Sovereignty signal detection.** Alongside its existing focuses, the Planner listens for signals that the User wants direct ownership over part of the work - domain preferences, areas the User has stressed about, things the User wants full control of, parts described with strong personal language, or explicit statements that the User intends to do certain work themselves. Sovereignty signals are not a separate question round and not a domain-by-domain procedure; they surface organically through the existing rounds. The Planner may make recommendations alongside its normal questioning when a signal appears (for example, flagging that based on what the User just said, certain Tasks may be candidates for direct ownership). The Planner remains a question-asker and does not pressure the User toward claiming work. Sovereignty signals are recorded as durable observations that survive into the Implementation Phase as Memory notes the Manager reads - first-class durable memory, not ephemeral planning context.
...
**User claims during review.** When the Planner presents the Plan and Spec for review, the User may choose to claim Tasks directly. Claimed Tasks are assigned to the User in the Plan instead of an AI Worker. The Spec does not change structurally as a result of User claims - the Spec captures project decisions, not execution distribution. Any rationale for User ownership belongs in Spec header notes only. If the User claims no Tasks at planning, the Plan looks identical to a non-collaborative Plan and the Implementation Phase begins as a normal session - until and unless the User claims something dynamically later.
...
**User-owned Task path** - When a User-owned Task becomes Ready, the Manager does not dispatch to a Task Bus. Instead, in the Manager's chat, it presents a Task Brief built from the same three-source synthesis as a Task Prompt (relevant Spec content, dependency context, Plan Task fields including objective, steps, guidance, output, and validation criteria), but written conversationally as a brief rather than as instructions to an executor. The Task Brief carries everything the User would need to do the work. The Manager then enters a standby collaborator posture for that Task while continuing to coordinate AI Workers on independent Tasks in parallel. See `{GUIDE_PATH:task-assignment}` for Task Brief construction.
...
**User takeover** - At any point during Worker execution, the User may signal takeover via natural language in the Worker's chat. There is no command for this. When the Worker recognizes a takeover signal, it pauses cleanly: it does not write a partial Task Log and does not report the takeover to the Manager at takeover time. The Worker enters a standby collaborator posture - available for execution-level questions, with full context of what was done so far, files touched, decisions made, and what was about to come next. The User does the remaining work however they choose. When the User reports back, the Worker proceeds to validation per `{GUIDE_PATH:task-execution}` and then to Task Logging.

**Reporting done** - When the User finishes a claimed Task (in either the Manager's chat or a Worker's chat), the User reports back via natural language. The report is expected to include what was done, any narrative context worth capturing, and validation status - which validation criteria the User checked themselves and which the User wants the hosting agent to handle. The hosting agent does not enforce a strict report format; it interprets the User's message, cross-references the Task's validation criteria, and proceeds to validation iteration. If the User's report omits validation status entirely, the hosting agent must explicitly check before proceeding rather than silently assuming completeness. The hosting agent peeks at the git diff (or equivalent ground-truth signal) before writing the log so the log accurately reflects what changed even if the User's narrative is incomplete.

**Validation iteration heuristic** - After the report, the hosting agent runs whichever validation steps remain. When validation fails on User-completed work, the hosting agent attempts to resolve the failure itself within a bounded scope: small surface area, mechanical or near-mechanical, no new design decisions, no domain logic the User owns. Within that scope, the hosting agent iterates - a small number of focused attempts, one targeted change per iteration, leveraging Ad-Hoc subagent dispatch (debug, research, verification) when the work would otherwise consume too much hosting-agent context. The hosting agent stops iterating and escalates to the User when the failure is systemic (architectural, design-level, multi-area, or otherwise outside the in-line correction scope) or persistent (the same issue is not resolving across iterations even when the agent understands it). Either condition triggers escalation as soon as it becomes clear; no fixed iteration count is required. When escalating, the agent presents what was attempted, what worked, what did not, hypotheses about why, and the cleaned-up state of the code: changes that produced real progress are kept; exploratory or unsuccessful changes that did not contribute are reverted. If nothing produced progress, the code returns to the exact state the User delivered. The User does not clean up after the agent. The User then decides whether to continue iterating or hand back to the agent with new direction. The two heuristics apply identically whether the host is a Manager or a Worker; default behavior is for the host to handle within its own scope and consult the other level only when the failure points there.

**Task Logging for User-owned Tasks** - The hosting agent writes the Task Log for any Task the User owned, at the standard Memory path for that Task (Stage directory, standard file naming). The log follows the standard Task Log format with additions: an ownership field indicating the User owned this Task, a takeover marker noting where the User took over and what was complete at that point if applicable, a breakdown of who did what during execution and validation (which steps the User performed, which the hosting agent performed during validation iteration, what was attempted, what was kept, what was reverted), and the User's narrative context. The log remains compatible with all downstream consumption (Stage summaries, dependency context for downstream Tasks, Handoffs) without special-casing. For Manager-hosted Tasks, the Manager writes the log directly to the standard Memory path; format and consumption are identical to a Worker-written log. For Worker-hosted Tasks, the Worker writes the log following its standard procedure with the same additions.
...
**Leftover handling heuristic** - When the Manager identifies a residual after Task Review (or during ongoing coordination), it judges whether the residual qualifies for inline handling. The qualifying scope is small surface area, mechanical or near-mechanical, no new design decisions, no new abstractions, and no domain logic that belongs to a Worker's specialization. Within that scope, the Manager can handle the residual itself directly (no Worker dispatch, no follow-up Task; action recorded in the parent Task's log or as a Tracker note) or offer the residual to the User. If the residual exceeds the qualifying scope, the Manager dispatches a follow-up to the appropriate Worker per existing review behavior, or escalates to the User for direction if the residual implies a Plan or Spec change. Choice between Manager-handles and offer-to-User is informed by the active recommendation behavior.

**Active recommendations** - The Manager actively recommends User involvement based on accumulated session signal: sovereignty signals captured during Context Gathering, claim and unclaim history during the session (what the User has taken on, what they have handed back, at what points), observed behavior patterns (how strict the User is about ownership in different domains, how the User handles validation, how engaged the User has been with various areas), and stated positions or concerns the User has expressed at any point. These inputs live as durable Memory notes in the Index and survive Manager Handoffs. Recommendation cadence: at Stage boundaries, actively - when a Stage completes and before the Manager begins dispatching the next Stage's Tasks, it reviews the upcoming Tasks against the recommendation inputs and proactively offers claims for relevant Tasks. In-Stage, quietly - the Manager only volunteers a recommendation when triggered by a concrete signal (a leftover that qualifies for offer-to-User, a follow-up that qualifies, a finding from Task Review touching an area the User has claimed sovereignty over, or similar). The Manager does not nag. Two hard constraints: the Manager never marks a Task as User-owned without explicit User confirmation, even when signals strongly point to it; and recommendations remain recommendations - the User can decline, defer, or counter at any time, and the Manager accepts the decision and proceeds.

**Followups and who handles them** - When Task Review reveals follow-up work that exceeds the leftover-handling scope, the Manager presents the follow-up to the User and indicates the available paths: dispatch to the responsible AI Worker, the User takes it themselves, or (if it actually does qualify under the leftover heuristic on closer look) the Manager handles it inline. The Manager's recommendation informs which path it leans toward, but the User makes the call.

### 7.4 Claim and Unclaim Mechanics

The User can claim any Task at any point. Whichever agent currently has context becomes the host for that Task's collaborative execution: the Manager for unassigned or planning-time-claimed Tasks, the Worker for in-progress Tasks. The User can unclaim any Task they hold. Whatever the User did so far becomes context the Manager packages into a normal Task Prompt for the responsible AI Worker, which then resumes the Task. If no Worker was originally assigned (the User claimed at planning), the Manager picks the best-fit Worker from the registry, possibly with User input. The User does nothing else. The User cannot claim and never return: agents need input to proceed, so a non-responding User leaves the relevant agent in standby. The Implementation Phase pauses on the User. There is no timeout, no auto-unclaim, and no escalation - those would violate ownership. A Task already marked Done is not claimable; if the User attempts it, the Manager notes the Task is complete and asks whether the User wants to revisit anything specific. Claims and unclaims happen via natural language in chat with the appropriate agent.

### 7.5 Stage Boundary Handling

At Stage boundaries, in addition to standard Stage-completion behavior (verification when needed, Stage summary written to the Index, proceeding to the next Stage's first dispatch), the Manager performs the active recommendation review described in §7.3 before beginning the new Stage's dispatches.
...
**Manager Handoff carries collaborative context** - In addition to the standard Manager Handoff content, the Manager's Handoff prompt and Handoff Log carry current Task ownership state for any User-held Tasks, sovereignty signals as they currently stand, observed User behavior patterns, and pending recommendations the Manager planned to surface but has not yet. The Index already carries some of this; the Handoff prompt fills in any working-knowledge gaps the Index does not capture. The new Manager instance, on initiation, reads the Index (which carries sovereignty signals and behavior notes near the top) and the Handoff prompt, and resumes both postures - coordinator toward Workers and collaborator toward the User - without dropping context on either side.
