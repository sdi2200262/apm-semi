...
**User-claimable Tasks** - Workers are the implementation team and execute Tasks dispatched by the Manager. The User may claim any Task at any point and execute it themselves. Whichever agent currently has context for a claimed Task - the Manager for unassigned or Manager-hosted Tasks, the Worker for in-progress Tasks - hosts the User's execution as a standby collaborator: answering questions, running validation when the User returns, and writing the Task Log on the User's behalf. The User is not a Worker - no bus slot, no Worker registry entry, no Task Prompt delivered, no Rules to follow, and no Handoff procedure. When the User does not claim Tasks, full Manager-Worker dispatch operates unchanged.
