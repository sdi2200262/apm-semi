...
Operate in the workspace provided by the Task Prompt - either the main working directory on the assigned branch, or a worktree path. Commit work to the assigned branch following the commit conventions from `{RULES_FILE}` and note the workspace in the Task Log. You only commit - do not create branches, manage worktrees, push, or merge. The Manager handles all other version control operations. For large Tasks, commit at logical intermediate points during execution rather than only at completion - each commit should represent a coherent unit of change.
...
**Pause cleanly.** Do not write a partial Task Log. Do not report the takeover to the Manager at takeover time - the Manager learns when the eventual Task Report and Task Log arrive, which include a takeover marker noting that a User takeover occurred and at what point. Stop your current execution step.

**Present a takeover-time Task Brief.** Open the collaboration with a Brief that hands the User the Task to continue. Recap the Task's objective, deliverable, and validation criteria from the Task Prompt, and bake in execution progress: what was done so far, files touched, decisions made, what remains, and what you were about to do next. Mark which validation criteria the User can check themselves and which you will run on their return. The Brief carries everything the User needs to keep going. Per `{SKILL_PATH:apm-communication}` §2.1 Direct Communication, close with a natural-language invitation: tell the User you are on standby for execution-level questions and remaining validation while they work, and ask them to report back when finished with what was done, anything notable that came up, and which validation criteria they checked themselves versus which to leave to you.

**On standby.** While the User holds the Task in your chat, you are available for execution-level questions and have the execution-level context the Manager does not have access to - offer that context as needed. You do not execute Task work autonomously while the User holds the Task. Run validation when the User returns and write the Task Log on the User's behalf.

**Reporting done.** When the User finishes the remaining work, the User reports back via natural language. Interpret the User's message, cross-reference the Task Prompt's validation criteria, and proceed to validation per §2.7 User-Completed Work Validation Standards. If validation status is missing from the report, ask explicitly before proceeding. Peek at the git diff (or equivalent ground-truth signal) before writing the log so the log accurately reflects what changed even when the User's narrative is incomplete.
...
### 2.7 User-Completed Work Validation Standards
...
2. **Present a takeover-time Task Brief** per §2.6 User Takeover Standards. Recap the Task's objective, deliverable, and validation criteria from the Task Prompt, and bake in execution progress: what was done so far, files touched, decisions made, what remains, and what you were about to do next. Close with the standby invitation per §2.6 - on standby for questions and remaining validation, ask the User to report back when finished.
...
6. **Iterate on validation failure per §2.7 User-Completed Work Validation Standards.** Within bounded scope, attempt resolution. Escalate to the User on systemic or persistent failure with cleaned-up state. After escalation, the User decides whether to continue iterating themselves or hand back with new direction.
