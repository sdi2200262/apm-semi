...
**User as valid assignee:** A Task may be assigned to the User instead of an AI Worker. APM Semi's typical pattern is the User taking on the critical or high-stakes Tasks they want full personal ownership over and AI Workers handling boilerplate or peripheral Tasks that depend on them - sovereignty signals from Context Gathering inform Worker-vs-User assignment during Plan Analysis per §3.2. User-owned Tasks participate in dependency, Stage, and parallel-dispatch logic identically to Worker-owned Tasks. The Worker registry is AI-only; the User is never added there. If the User claiming all Tasks in a Worker's domain leaves that Worker with nothing to do, remove the Worker from the registry.
...
1. Analyze work structure from gathered context and the approved Spec per §2.2 Decomposition Principles:
   ...
   - *Stage N.* For each Stage:
     ...
     - *Per-Task analysis.* For each Task:
       ...
       - *Sovereignty consideration:* assess whether this Task should be assigned to the User instead of an AI Worker per §2.4 Plan Standards. Reason from the sovereignty signals captured during Context Gathering - does this Task fall in an area the User signaled wanting direct ownership over, an area the User described as critical or stressed about, or an area the User explicitly said they would build themselves? If the alignment is clear, set the assignee to User. If the alignment is borderline or partial, leave the Task assigned to its Worker and note the signal so it can be surfaced as a recommendation during Plan review. When no signal touches this Task, no decision is needed - it stays Worker-assigned.
...
3. Pause for User review:
   ...
   - Ask User to review the Plan. The User can request modifications and may also claim any additional Task directly (or unclaim a Task already assigned to them) - claim and unclaim are conversational, the User simply says which Tasks they want to take or hand back. Surface borderline sovereignty signals from Context Gathering as recommendations alongside.
   - If claim or unclaim changes are made, update each affected Task's assignee per §4.2 Plan Format, remove any Worker that ends up with no Tasks from the Workers field, and regenerate the Dependency Graph node labels accordingly. Add a Plan note recording the change and any rationale the User provided.
   - Record any sovereignty observations from Context Gathering that did not translate into User-owned Tasks (borderline signals, partial preferences, contextual rationale) in Plan notes per §4.2 Plan Format so the Manager has them as awareness during the Implementation Phase.
   - If modifications needed, apply and repeat step 3.
...
- *Planner notes:* Placed immediately after the horizontal rule separator, before Stage sections. Use the format `> **Notes:** <prose or unordered list>`. These cover what you observed about the work structure per §2.1 Workflow Context - why boundaries exist, natural groupings or sequencing patterns, critical path, convergence points, and Stage boundaries where holistic verification may be valuable. Sovereignty observations from Context Gathering that did not translate into User-owned Tasks (borderline signals, partial preferences, contextual rationale) also live here, so the Manager has them as awareness during the Implementation Phase.
