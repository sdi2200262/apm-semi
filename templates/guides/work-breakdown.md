...
- *User-owned Task collaboration (Manager side).* When a Task is owned by the User, deliver a Task Brief in chat instead of dispatching to a Task Bus, hold a standby collaborator posture while the User works, run validation iteration on the User's behalf when the User reports done, and write the Task Log on the User's behalf at the standard Memory path. Continue dispatching independent Worker Tasks in parallel.
...
2. Read `{RULES_FILE}` (or confirm it does not exist), then write the APM_RULES block per §4.3 APM_RULES Block:
   ...
   - Always include the baseline `## Collaborative Execution` section covering User takeover (Worker side), User-owned Task collaboration (Manager side), validation iteration, leftover handling, and active recommendations per §2.5 `{RULES_FILE}` Standards.
3. Pause for User review:
