...
4. Reconstruct collaborative coordination state: for the Manager, read sovereignty signals, User behavior patterns, and User preferences from the Index Memory Notes, check the Tracker's Owner column for any User-held Tasks, and look for in-progress Task Briefs in chat history; resume both postures (dispatcher toward Workers, collaborator toward the User). For a Worker that was paused for a User takeover, recognize that state from chat history and resume the standby posture.
